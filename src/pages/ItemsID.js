            ...
            <div>
                        ...
                        <div className="card-body-link" >
                            <div className="card mb-5 card-profil" >
                                <div className="row no-gutters" >
                                    <div className="row no-gutters" >
                                        <img src={process.env.REACT_APP_API_URL + this.props.data_item.images} className="card-img card-img-detail"
                                            alt="..." />
                                    </div>
                                    <div className="col-md-6" >
                                        <div className="card-body" >
                                            <h5 className="cart-titles" > {this.props.data_item.name_restaurant} </h5> <hr />
                                            <h6 className="cart-resto" > {this.props.data_item.name_item} - {this.props.data_item.location} </h6>
                                            <p className="text-muted" > {this.props.data_item.desciption} </p> <h6 className="cart-price" > Rp. {this.props.data_item.price} </h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="container mt-5" >
                    <Tabs defaultIndex={1} onSelect={index => console.log(index)} >
                        ...
                        <TabPanel > {
                            this.props.data_review && this.props.data_review.map((val, idx) => (<Reviews name={val.name_user} date_created={val.date_created} review={val.review} date={val.date_created} images={process.env.REACT_APP_API_URL + val.images} />))}
                            <div className="row justify-content-center" >
                                <div className="text-center" > < textarea onChange={e => this.handleTextComment(e)} name="comment" className="form-comment form-control pl-3" rows="2"
                                    placeholder="comment ..." > </textarea></div >
                            </div>
                            <div className="col-lg-5 justify-content-center text-center mt-3" >
                                <button type="button" onClick={e => this.handleComment(e)} className="btn btn-danger">Comment</button> </div>
                        </TabPanel>
                    </Tabs>
                </div>
                { /* Add Items Hide*/}
                                ...
                                <div class="row no-gutters" >
                                    ...
                                    <div class="col-md-8 text-left" >
                                        <div class="card-body card-bodies" >
                                            <h6 className="cart-prices" > Rp. {this.props.data_item.price} <span > /item</span >
                                            </h6>
                                            <input type="number" onChange={e => this.handleValue(e)} name="value"
                                                className="cartvalue form-control" min="1"
                                                placeholder="add items..." />
                                            <div className="valuealign" > < button onClick={e => this.handleAddToCart(e)} type="button" className="btn-carts btn-auth btn btn-warning mt-3" > Save </button>
                                            </div >
                                        </div>
                                    </div>
                                </div>
